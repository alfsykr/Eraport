                      ...
                      <tr>
                        ...
                        <td>
                          <button type="button" class="btn btn-warning btn-sm" data-toggle="modal"
                            data-target="#modal-edit{{$tapel->id}}">
                            <i class="fas fa-pencil-alt"></i>
                          </button>
                          @if($tapel->id != session()->get('tapel_id'))
                            <form action="{{ route('tapel.destroy', $tapel->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
                              <button type="submit" class="btn btn-danger btn-sm"
                                onclick="return confirm('Hapus Tahun Pelajaran {{$tapel->tahun_pelajaran}}?\n\nPeringatan: Pastikan tidak ada kelas yang menggunakan tahun pelajaran ini.')">
                            ...
                            <form action="{{ route('tapel.update', $tapel->id) }}" method="POST">
                              ...
                              <div class="modal-body">
                                <div class="form-group row">
                                  ...
                                  <div class="col-sm-9">
                                    <input type="text" class="form-control" id="tahun_pelajaran" name="tahun_pelajaran"
                                      placeholder="Tahun Pelajaran" value="{{$tapel->tahun_pelajaran}}">
                                  </div>
                                </div>
                                <div class="form-group row">
                                  <label for="semester" class="col-sm-3 col-form-label">Semester</label>
